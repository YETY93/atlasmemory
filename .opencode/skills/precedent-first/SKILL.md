...
description: >
  ...
  (workspace_search / workspace_get) para ver si el mismo
...
## Tools (plugin `workspace.ts`)

| Tool | Uso |
|------|-----|
| `workspace_search` | ¿Existe algo así en otro proyecto? |
| `workspace_get` | Detalle completo de un componente ajeno |
| `workspace_projects` | Proyectos registrados y frescura |
| `workspace_reindex` | Reconstruir el índice |

Si las tools no aparecen, revisá que exista `.opencode/tools/workspace.ts` en el repo
y reiniciá OpenCode.
...
- `AGENTS.md` — reglas e invariantes del proyecto
